...
# mindis= set of distances between points and closest centers, wp= set of weights
# returns index of the selected point
def randomPoint(p,mindis,wp):
    probSum = 0
    randValue = random.random() # random value between 0 and 1
    sumation = 0
    for i,q in enumerate(p):
        sumation += mindis[i]*wp[i]
        res=0
    for i, point in enumerate(p):
        probSum += wp[i]*mindis[i]/sumation
...
def kmeansPP(p,wp,k,iter):
    ...
    mindis = np.linalg.norm(pc - s[0], axis=1)
    for i in range(1,k):
        randPoint = randomPoint(pc,mindis,wpc)
        s[i] = np.copy(pc[randPoint])
        for index,point in enumerate(pc):
            newMinds = distance(point,s[i])
            if(newMinds < mindis[index]):
                mindis[index] = newMinds
